...
function GachaDetailExperience({ gachaId }: Props) {
  ...
  return (
    <div className="space-y-4">
      {sessionId ? (
        <MultiGachaSession sessionId={sessionId} onFinished={() => setFinished(true)} />
      ) : (
      ...
      {sessionId && finished && (
        <div className="text-center">
          <button
            ...
            className="inline-flex items-center justify-center rounded-full border border-white/15 px-4 py-3 text-[11px] uppercase tracking-[0.35em] text-white transition hover:border-neon-blue"
          >
            同じガチャでもう一度回す
          </button>
        </div>
